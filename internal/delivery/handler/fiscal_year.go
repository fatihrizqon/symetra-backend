...
func (h *FiscalYearHandler) Activate(ctx fiber.Ctx) error {
	...
	return ctx.Status(fiber.StatusOK).JSON(response.JSON{Status: fiber.StatusOK, Message: "Fiscal Year activated", Data: result})
}

func (h *FiscalYearHandler) ClosePeriod(ctx fiber.Ctx) error {
	companyID, err := util.GetCompanyID(ctx)
	if err != nil {
		util.HandleError(ctx, fiber.StatusBadRequest, err)
		return nil
	}
	periodId, err := uuid.Parse(ctx.Params("period_id"))
	if err != nil {
		util.HandleError(ctx, fiber.StatusBadRequest, err)
		return nil
	}
	...
	return ctx.Status(fiber.StatusOK).JSON(response.JSON{Status: fiber.StatusOK, Message: "Period closed", Data: result})
}
